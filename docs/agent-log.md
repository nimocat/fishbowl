...
- Installed acceptance: production migrated backup-first from schema 7 to 8 with `quick_check=ok` and unchanged 1 project / 1,350 events / 82 Cases / 476 nodes before observations. The final real task observation returned successfully in 6.73 seconds, scanned 250,000 entries per snapshot, attributed 201 bytes across two relative generated/dependency paths, and conservatively marked both `review` because the scan was truncated. No cleanup ran.
