...
fn project_registration_and_update_are_atomic_canonical_and_idempotent() {
    ...
    let root = std::env::temp_dir().join(format!("fishbowl-project-root-{}", std::process::id()));
    let alias = std::env::temp_dir().join(format!("fishbowl-project-alias-{}", std::process::id()));
...
fn snapshot_export_import_is_bounded_redacted_atomic_and_idempotent() {
    ...
    let source_root =
        std::env::temp_dir().join(format!("fishbowl-snapshot-source-{}", std::process::id()));
    let target_root =
        std::env::temp_dir().join(format!("fishbowl-snapshot-target-{}", std::process::id()));
...
fn database(label: &str) -> std::path::PathBuf {
    let path =
        std::env::temp_dir().join(format!("fishbowl-write-{label}-{}.db", std::process::id()));
    let connection = Connection::open(&path).unwrap();
    connection
        .execute_batch(
