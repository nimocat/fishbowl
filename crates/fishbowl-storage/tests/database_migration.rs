...
use fishbowl_storage::{DatabaseFaultPoint, DatabaseManager};
use rusqlite::Connection;

#[test]
fn new_database_is_schema_v9_safe_and_integral() {
    let path = path("new");
    let managed = DatabaseManager::open(&path).unwrap();
    assert_eq!(managed.user_version().unwrap(), 9);
...
fn integrity_check_is_read_only_while_the_managed_database_is_online() {
    ...
    assert_eq!(
        managed.user_version().unwrap(),
        fishbowl_storage::SCHEMA_VERSION
    );
    assert_eq!(
...
fn path(label: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!("fishbowl-schema-{label}-{}.db", std::process::id()))
}
