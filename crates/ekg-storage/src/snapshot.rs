...
fn resolve_project(c: &Connection, r: &ProjectReference) -> Result<String, WriteError> {
    let by_id = r
        ...
        .transpose()?
        .flatten();
    let by_root=r.project_root.as_ref().map(|root|c.query_row("SELECT projects.id FROM projects LEFT JOIN project_aliases ON project_aliases.project_id=projects.id WHERE canonical_root=? OR project_aliases.root=?",params![root,root],|x|x.get(0)).optional()).transpose()?.flatten();
    match (by_id, by_root) {
